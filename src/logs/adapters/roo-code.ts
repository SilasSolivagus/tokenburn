...
function getRooCodeDir(): string {
  ...
  if (process.platform === 'darwin') {
    return path.join(home, 'Library', 'Application Support', 'Code', 'User', 'globalStorage', 'rooveterinaryinc.roo-cline')
  }
  return path.join(home, '.config', 'Code', 'User', 'globalStorage', 'rooveterinaryinc.roo-cline')
}

export const rooCodeAdapter: ImportAdapter = {
  name: 'roo-code',

  detect() {
    return fs.existsSync(path.join(getRooCodeDir(), 'tasks'))
  },

  scan() {
    const tasksDir = path.join(getRooCodeDir(), 'tasks')
  ...
  parse(filePath: string, offset: number = 0): LogEntry[] {
    ...
    for (const event of events) {
      ...
      entries.push({
        uuid: `roo-code-${taskId}-${timestamp}`,
        parentUuid: null,
        sessionId: taskId,
        timestamp,
        model: data.model ?? '',
